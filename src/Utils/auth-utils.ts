...
export function makeCacheableSignalKeyStore(
    ...
    function getUniqueId(type: keyof SignalDataTypeMap | string, id: string) {
		return `${type}.${id}`
	}
	...
	return {
        async get<T extends keyof SignalDataTypeMap>(type: T, ids: string[]) {
			return cacheMutex.runExclusive(async () => {
				...
				for (const id of ids) {
                    const item = cache.get<SignalDataTypeMap[T] | null>(getUniqueId(type, id))
        ...
        async set(data: SignalDataSet) {
			return cacheMutex.runExclusive(async () => {
                ...
                for (const key in data) {
                    const typedKey = key as keyof SignalDataTypeMap
                    const byType = data[typedKey]
                    if (!byType) continue
                    for (const id in byType) {
                        const value = byType[id]
                        if (value !== null && value !== undefined) {
                            cache.set(getUniqueId(typedKey, id), value as any)
                            keys += 1
                        }
                    }
                }
 ...
 */
type StoreKey = keyof SignalDataTypeMap
type StoreKeyOrSigned = StoreKey | 'signed-pre-key'

type Bucket<K extends StoreKey> = { [id: string]: SignalDataTypeMap[K] | null }
function ensureBucket<K extends StoreKey>(set: SignalDataSet, key: K): Bucket<K> {
    if (!set[key]) {
        ;(set as any)[key] = {}
    }
    return set[key] as Bucket<K>
}

async function handlePreKeyOperations<
    K extends StoreKeyOrSigned
>(
    data: SignalDataSet,
    keyType: K,
    ...
    const mutex = getPreKeyMutex(keyType as string)

	await mutex.runExclusive(async () => {
        const keyData = (data as any)[keyType] as Record<string, any> | undefined
		if (!keyData) return

        // Ensure structures exist (only for real store keys)
        const realKey = (keyType === 'signed-pre-key' ? undefined : (keyType as StoreKey))
        const tBucket = realKey ? ensureBucket(transactionCache, realKey) : undefined
        const mBucket = realKey ? ensureBucket(mutations, realKey) : undefined
        ...
        for (const keyId of updateKeys) {
            if (tBucket && mBucket) {
                tBucket[keyId] = keyData[keyId] as any
                mBucket[keyId] = keyData[keyId] as any
            }
        }
            ...
            if (tBucket && mBucket) {
                for (const keyId of deletionKeys) {
                    if (tBucket[keyId]) {
                        ...
                        logger.warn(`Skipping deletion of non-existent ${String(keyType)} in transaction: ${keyId}`)
                    }
                }
            }
        ...
        const lookupType = (keyType === 'signed-pre-key' ? 'pre-key' : (keyType as StoreKey))
        const existingKeys = await state.get(lookupType, deletionKeys)
        if (tBucket && mBucket) {
            for (const keyId of deletionKeys) {
                if ((existingKeys as any)[keyId]) {
                    tBucket[keyId] = null
                    mBucket[keyId] = null
                } else {
                    logger.warn(`Skipping deletion of non-existent ${String(keyType)}: ${keyId}`)
                }
            }
        }
	})
 ...
 */
function handleNormalKeyOperations<K extends StoreKey>(
    data: SignalDataSet,
    key: K,
    transactionCache: SignalDataSet,
    mutations: SignalDataSet
) {
    const tBucket = ensureBucket(transactionCache, key)
    const mBucket = ensureBucket(mutations, key)
    Object.assign(tBucket, data[key])
    Object.assign(mBucket, data[key])
}
...
async function processPreKeyDeletions(
	...
	await mutex.runExclusive(async () => {
		...
		for (const keyId in keyData) {
			if (keyData[keyId] === null) {
                const lookupType = (keyType === 'signed-pre-key' ? 'pre-key' : keyType)
                const existingKeys = await state.get(lookupType, [keyId])
				if (!existingKeys[keyId]) {
					logger.warn(`Skipping deletion of non-existent ${keyType}: ${keyId}`)
                    delete (data as any)[keyType][keyId]
				}
			}
		}
	})
...
async function withMutexes<T>(
    ...
    if (keyTypes.length === 1) {
        return getKeyTypeMutex(keyTypes[0] as string).runExclusive(fn)
    }
	...
	// Then acquire all mutexes in order using Promise.all for better efficiency
                ...
                const mutexes = sortedKeyTypes.map(k => getKeyTypeMutex(k || ''))
	...
	return {
		get: async (type, ids) => {
			if (isInTransaction()) {
                ...
                return ids.reduce<{ [id: string]: SignalDataTypeMap[typeof type] }>((dict, id) => {
                    const value = (transactionCache[type] as any)?.[id] as SignalDataTypeMap[typeof type] | undefined
					if (value) {
						dict[id] = value
					}
				...
				if (type === 'sender-key') {
                    ...
                    const results: Record<string, SignalDataTypeMap[typeof type]> = {}
		...
		set: async data => {
			if (isInTransaction()) {
                ...
                for (const key in data) {
                    const typedKey = key as keyof SignalDataTypeMap
                    if (!transactionCache[typedKey]) {
                        (transactionCache as any)[typedKey] = {}
                    }

					// Special handling for pre-keys and signed-pre-keys
                    if (key === 'pre-key' || key === 'signed-pre-key') {
                        await handlePreKeyOperations(data, key as 'pre-key' | 'signed-pre-key', transactionCache, mutations, logger, true)
					} else {
						// Normal handling for other key types
                        handleNormalKeyOperations(data, typedKey, transactionCache, mutations)
					}
				}
			} else {
                    ...
                    for (const senderKeyName of senderKeyNames) {
						await queueSenderKeyOperation(senderKeyName, async () => {
                            ...
                            const senderKeyData: SignalDataSet = {
								'sender-key': {
                                    [senderKeyName]: (data['sender-key'] as any)?.[senderKeyName] ?? null
								}
							}
                ...
                const nonSenderKeyData: SignalDataSet = { ...data }
                        ...
                        await withMutexes(Object.keys(nonSenderKeyData), getKeyTypeMutex, async () => {
                            ...
                            for (const keyType in nonSenderKeyData) {
                                if (keyType === 'pre-key' || keyType === 'signed-pre-key') {
                                    await processPreKeyDeletions(nonSenderKeyData, keyType as 'pre-key' | 'signed-pre-key', state, logger)
								}
							}
                    ...
                    await withMutexes(Object.keys(data), getKeyTypeMutex, async () => {
                        ...
                        for (const keyType in data) {
                            if (keyType === 'pre-key' || keyType === 'signed-pre-key') {
                                await processPreKeyDeletions(data, keyType as 'pre-key' | 'signed-pre-key', state, logger)
							}
						}
